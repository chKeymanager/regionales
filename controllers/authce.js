...
conexion.query("SELECT * FROM rep_fact WHERE clave_del_producto = 'INSCAT023' AND fecha_factura > '2023-01-01 00:00:00'", (error, results, fields) => {
    if (error) {
...
const mysqlcer = (req,res) => {
  ...
  conexion.query("SELECT * FROM rep_fact WHERE estado = 'Pagado' AND (clave_del_producto = 'CAT002023' OR clave_del_producto = 'CAT012023') AND fecha_factura > '2023-01-01 00:00:00'", (error, results, fields) => {
      if (error) {
        console.error('Error executing query: ' + error.stack);
        return;
      }
